...
describe("release decision policy", () => {
  ...
  it.each(["macos-node", "checks-node-core-test-nondist-shard", "openclaw/ci-gate"])(
    ...
    (name) => {
      const result = classifyReleaseSnapshot({
        children: [
          child("normalCi", {
            conclusion: "failure",
            jobs: [...nativeCiJobs, { name, conclusion: "failure", status: "completed" }],
            status: "completed",
          }),
        ],
        releaseProfile: "stable",
      ...
      expect(result.blockers.map((blocker) => blocker.job)).toEqual([name]);
