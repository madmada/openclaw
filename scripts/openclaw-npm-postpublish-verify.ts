...
export function collectInstalledRootDependencyManifestErrors(
  ...
  const legacyRootRelativeImportsByFile = new Map<string, string[]>();
  ...
  for (const filePath of distFiles.files) {
    ...
    if (owners) {
      ...
      const legacyOwnership = collectLegacyGeneratedOwnership(file.source);
      if (legacyOwnership.hasExtensionOwnedImports) {
        legacyExtensionsByFile.set(file.relativePath, legacyOwnership.extensionsByDependency);
      }
      if (legacyOwnership.rootRelativeImports.length > 0) {
        legacyRootRelativeImportsByFile.set(file.relativePath, legacyOwnership.rootRelativeImports);
      }
    }
  }
  ...
  const pending = [...importsByFile.keys()].filter(
    (file) => !extensionsByFile.has(file) && !legacyExtensionsByFile.has(file),
  );
  const visited = new Set<string>();
  const distDir = importsByFile.size ? realpathSync(join(packageRoot, "dist")) : "";
  const enqueueRelativeImport = (file: string, specifier: string) => {
    try {
  ...
  // Legacy region ownership is occurrence-based. An unowned relative edge in an
  // otherwise plugin-owned chunk still makes its target root-reachable.
  for (const [file, imports] of legacyRootRelativeImportsByFile) {
    for (const specifier of imports) {
      enqueueRelativeImport(file, specifier);
    }
  }
  while (pending.length > 0) {
    ...
    legacyExtensionsByFile.delete(file);
...
type LegacyGeneratedOwnership = {
  extensionsByDependency: Map<string, Set<string>>;
  hasExtensionOwnedImports: boolean;
  rootRelativeImports: string[];
};

function collectLegacyGeneratedOwnership(source: string): LegacyGeneratedOwnership {
  ...
  const rootRelativeImports: string[] = [];
  let hasExtensionOwnedImports = false;
  for (const runtimeImport of imports) {
    const extensionId = ownersByPosition.get(runtimeImport.start);
    hasExtensionOwnedImports ||= extensionId !== undefined;
    if (!extensionId && runtimeImport.specifier.startsWith(".")) {
      rootRelativeImports.push(runtimeImport.specifier);
    }
  ...
  return {
    extensionsByDependency: ownersByDependency,
    hasExtensionOwnedImports,
    rootRelativeImports,
  };
}
